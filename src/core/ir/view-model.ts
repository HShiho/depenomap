...
export interface ViewModel {
   ...
   *
   * **すべてのファイルノードが必ずバケットを持つ**（メソッドが 0 件なら空配列）。
   * `nodesByLayer` と同じ規則であり、引き当ての結果が undefined になる経路は無い。
   *
   * **キーの並びは正本 JSON のファイル順ではない。** メソッドが最初に現れた順に
   * 並び、メソッドを持たないファイルが末尾に来る。並び順が要るときは `nodes.file`
   * を回してこのマップを引く。
   */
