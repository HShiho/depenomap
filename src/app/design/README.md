...
- `bg-red-500` や `text-sm` は**存在しない**（ビルドが通っても何も効かない）
- 使えるのは、この README の対応表にある名前だけ

「直値を書かない」を規約の暗記ではなく、書けない状態として担保するための措置。
...
数値には `.tabular`（`font-variant-numeric: tabular-nums`）を付ける。件数やズーム率が
更新されたときに桁が揺れない。
