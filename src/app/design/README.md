...
| （なし）                         | `--overlay-inset`                      | （変数のみ）                  | キャンバスに重ねる要素の端からの距離（UT-05）       |
| （なし）                         | `--toolbar-inset`                      | （変数のみ）                  | キャンバス下部のツールバーの下端からの距離（UT-05） |
...
- **描き分けを足したら、`src/app/legend/legend-items.ts` に 1 行足す**（UT-26）。凡例はその定義から描くので、足し忘れると図には出ているのに凡例に無い状態になる
