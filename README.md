...
**ファイル単位のノードマップが出る。** 層ごとに列を作り、依存を「使う側 → 使われる側」
の矢印で描く。ノードをクリックすると選択され、背景をクリックすると解除される。

まだ載っていないのは、メソッド粒度の切り替え・列の軸の切り替え・検索・
サイドバーの一覧・概要パネル・トラックパッド操作・手でのノード移動である。
...
```
    ...
    [済] デザイン基盤（トークン・テーマ）
    [済] 画面の骨格と表示状態
    [済] ノードマップの描画（ファイル粒度）      ← いまここ
    [  ] 粒度の切り替え・列の軸・検索・サイドバー
```
