...
実装宛で読んでいるあいだは、**インターフェースのノードを畳める**。その読み方では
呼び出しの線が実装へ向かうので、インターフェースは線の通らない点として残る。畳んでも
呼び出しは図に残り、消えるのは `implements` の線（誰が実装か）だけ。畳んでいることと、
その線が出ないことは左上に出る。インターフェース宛で読んでいるあいだは畳めない
（畳むと呼び出しの事実が図から消えるため）。畳んだかどうかは解析対象ごとに覚える。
...
```
    ...
    [済] ノードから VSCode で開く
    [済] 凡例と、描き方についての断り
    [済] 経由の呼び出しの読み方（interface / 実装）
    [済] interface のノードを畳む                ← いまここ
```
